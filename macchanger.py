...
def get_arguments():
    ...
    parser.add_option("-m", "--mac", dest="new_mac", help="Set your new mac , leave it for a random generated mac")
    parser.add_option("-v", "--verbose", action="store_true", dest="verbose", default="false", help="Activate verbose mode")
    parser.add_option("-l", "--legacy", action="store_true", default="false", dest="legacy", help="Legacy, use old ifconfig instead of ip")

    (options, arguments) = parser.parse_args()
    if not options.interface:
        parser.error("[-] Please specify an interface -i [INTERFACE], use --help for more info.")
        exit(0)
    elif not options.new_mac:
        parser.error("[-] Please specify a mac address or autogen a random mac using -m option, use --help for more info.")
        exit(0)
    return options
...
def change_mac(interface, new_mac):
    print("[+] Changing MAC address for " + interface + " to " + new_mac + " using ip")
    subprocess.call(["ip", "link", "set", interface, "down" ])
    subprocess.call(["ip", "link", "set", interface, "address", new_mac ])
    subprocess.call(["ip", "link", "set", interface, "up" ])

def change_mac_legacy(interface, new_mac):
    print("[+] Changing MAC address for " + interface + " to " + new_mac + " using ifconfig")
...
def get_current_mac(interface,legacy):
    if legacy == True :
        ...
        ifconfig_result = subprocess.check_output(["ip", "link", "show", interface])
